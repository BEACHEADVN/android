...
for i in wget aapt perl zip python
do
...
for i in aov.sh bt.sh gi.sh r.sh termux.sh magisk.sh ytm.sh core.sh adaway.sh ra.sh pubg.sh yi.sh gps.sh tap.py tap_avatar.py tap_text.py tap_update.py
do
...
for i in tap.py tap_avatar.py tap_text.py tap_update.py
do
	if [ ! -f /storage/emulated/0/Download/ext/temp/$i ]
	then
		echo "$i command not found in temp folder. Cannot copy!"
