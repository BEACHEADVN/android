...
for pkg in $PKGS
do
    if su -c "pm list packages | grep -q $pkg"
    then
        FOUND=true
        su -c "pm uninstall -k $pkg"
        echo "•Uninstall $pkg" >> /storage/emulated/0/Download/ext/temp/log.txt
    fi
done
...
then
    ...
    s/ Sat, / thứ Bảy, /g;
    s/ Sun, / Chủ nhật, /g
    ' /storage/emulated/0/Download/ext/temp/log.txt
fi
